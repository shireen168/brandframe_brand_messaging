...
export function SavedBrandDocs({ docs }: Props) {
  ...
  return (
    <div className="space-y-3">
      {localDocs.map((doc, i) => (
        <motion.div key={doc.id} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }}
          ...
          <button onClick={() => setExpandedId(expandedId === doc.id ? null : doc.id)}
            ...
            <span className={`text-lg text-white/30 transition-transform ${expandedId === doc.id ? 'rotate-180' : ''}`}>↓</span>
          </button>
